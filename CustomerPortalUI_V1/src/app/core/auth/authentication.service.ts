...
import { UserRegisration } from '../models/user-management/user-registration';
import { SessionStorage } from './session-storage.service';

@Injectable()
export class AuthenticationService implements AuthService {
	API_URL = 'http://loanapi.pyxidaapps.com';
	API_ENDPOINT_TOKEN = '/token';
	API_ENDPOINT_LOGIN = '/login';
	API_ENDPOINT_REFRESH = '/refresh';
	API_ENDPOINT_REGISTER = '/api/system-settings/users/register';

	public onCredentialUpdated$: Subject<AccessData>;
	private headers = new Headers({'Content-Type': 'application/json', 'Access-Control-Allow-Origin': 'http://loanapi.pyxidaapps.com'});
	...
	public login(credential: Credential): Observable<any> {
		...
		// return this.http.get<AccessData>(this.API_URL + this.API_ENDPOINT_TOKEN + '?' + this.util.urlParam(credential)).pipe(
		// 	map((result: any) => {
		...
		return this.http.post(this.API_URL + this.API_ENDPOINT_TOKEN, credential)
			.pipe(map((result: any) => {
				if (result instanceof Array) {
					return result.pop();
				}
	...
	private saveAccessData(accessData: AccessData) {
		if (typeof accessData !== 'undefined') {
			...
			if(accessData.systemUser !== undefined && accessData.systemUser !== null) {
				this.sessionStorage.setSystemUser(accessData.systemUser);
			}
			// this.tokenStorage
	 ...
	 * @param {UserRegisration} credential
	 * @returns {Observable<any>}
	 */
	public register(credential: UserRegisration): Observable<any> {
